...
import { inspectRecipientWorkbook } from "@/lib/email-campaigns/recipient-import";

const MAX_UPLOAD_BYTES = 5 * 1024 * 1024;

/** Step 1 of the recipient Excel import: headers + a few sample rows for the mapping step. Writes nothing. */
export async function POST(request: NextRequest) {
  try {
    ...
    let inspected;
    try {
      inspected = await inspectRecipientWorkbook(Buffer.from(await file.arrayBuffer()));
    } catch {
      return NextResponse.json({ error: "That file couldn't be read as an Excel workbook. Save it as .xlsx and try again." }, { status: 422 });
    }
    if (inspected.headers.length === 0) return NextResponse.json({ error: "The workbook has no recognizable header row." }, { status: 422 });
